...
async def notify_admin(context: ContextTypes.DEFAULT_TYPE, message: str):
    ...
    if admin_id:
        try:
            await context.bot.send_message(chat_id=admin_id, text=f"Error Alert: {message}")
        except Exception as e:
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        per_message=False  # Explicitly set to default to avoid warnings
    )
    ...
    try:
        ...
        # Notify admin on crash (if configured)
        if os.getenv("ADMIN_TELEGRAM_ID"):
            asyncio.run(notify_admin(application, f"Bot crashed: {str(e)}"))
        sys.exit(1)

async def notify_admin(application: Application, message: str):
    """Notify the admin of critical errors."""
    admin_id = os.getenv("ADMIN_TELEGRAM_ID")
    if admin_id:
        try:
            await application.bot.send_message(chat_id=admin_id, text=f"Error Alert: {message}")
        except Exception as e:
            logger.error(f"Failed to notify admin: {e}", exc_info=True)
