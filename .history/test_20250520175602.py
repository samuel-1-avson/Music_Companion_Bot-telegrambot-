...
# Example for get_spotify_token
async def get_spotify_token_async() -> Optional[str]:
    # ... (similar logic but using await client.post(...)) ...
...
async def recommend_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # In recommend_music, inside the elif for playlists:
# token = get_spotify_token() # WRONG
user_s_data = user_contexts[user_id].get("spotify", {})
user_access_token = user_s_data.get("access_token")
if user_s_data.get("expires_at") and datetime.now(pytz.UTC).timestamp() > user_s_data["expires_at"]:
    user_access_token = refresh_spotify_token(user_id) # This needs to be async if spotify funcs are async

if not user_access_token:
    logger.warning(f"User {user_id} has no valid access token for fetching playlist tracks.")
else:
    headers = {"Authorization": f"Bearer {user_access_token}"} # CORRECT
    # ... rest of the requests.get call ...
...
async def link_spotify(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    auth_url = (
        ...
        f"&state={user_id}"
        # In link_spotify function
        "&scope=user-read-recently-played%20user-top-read%20playlist-read-private"
    )
    keyboard = [
...
@lru_cache(maxsize=50) # Add caching for lyrics
def get_lyrics(song_title: str, artist_name: Optional[str] = None) -> str:
    ...
    try:
        ...
        if song_obj and song_obj.lyrics:
            # Basic cleaning of common Genius artifacts
            lyrics = re.sub(r'\[.*?\]\n?', '', song_obj.lyrics) # Remove [Chorus], [Verse], etc. and following newline
            lyrics = re.sub(r'\d*EmbedShare URLCopyEmbedCopy', '', lyrics) # Remove Genius footer
            lyrics = re.sub(r'\n{3,}', '\n\n', lyrics.strip()) # Normalize multiple newlines
            return lyrics if lyrics else "Lyrics not found or are empty."
        return "Lyrics not found."
    except Exception as e: # More specific exceptions could be caught from lyricsgenius if known
        logger.error(f"Error fetching lyrics from Genius for '{song_title}' by '{artist_name}': {e}")
