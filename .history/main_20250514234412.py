...
async def download_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle music download requests with robust error handling."""
    user_id = update.effective_user.id
    
    if user_id in active_downloads:
        await update.message.reply_text("⚠️ You have a download in progress. Please wait.")
        return
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            title = info.get('title', 'Unknown Title').replace("/", "-")  # Sanitize filename
        ...
        for ext in ['mp3', 'm4a', 'webm']:
            possible_file = os.path.join(download_dir, f"{title}.{ext}")
        ...
        with open(audio_file, 'rb') as audio:
            await update.message.reply_audio(
                audio=audio,
                title=title[:64],  # Telegram has 64-char limit for titles
        ...
        if "Private video" in error_msg:
            ...
            await update.message.reply_text("⚠️ This content is copyright protected.")
        elif "Unsupported URL" in error_msg:
            await update.message.reply_text("❌ Unsupported URL. Try a different video.")
        else:
            await update.message.reply_text(f"❌ Download error: {error_msg[:200]}")
