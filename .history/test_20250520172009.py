...
SPOTIFY_REDIRECT_URI = os.getenv("SPOTIFY_REDIRECT_URI") # Ensure this is set and matches dashboard
...
async def ensure_spotify_token_valid(user_id: int) -> bool:
    if is_user_spotify_linked(user_id): return True
    context_data = user_contexts.get(user_id, {})
    if context_data.get("spotify", {}).get("refresh_token"):
        ...
        refreshed_token = await refresh_spotify_token(user_id)
        if refreshed_token: logger.info(f"Spotify token successfully refreshed for user {user_id}."); return True
        else:
...
def search_spotify_track_sync(token: str, query: str, type: str = "track") -> Optional[Dict]: 
    ...
    url = "https://api.spotify.com/v1/search"; headers = {"Authorization": f"Bearer {token}"}; params = {"q": query, "type": type, "limit": 1}
    try:
        response = requests.get(url, headers=headers, params=params, timeout=10); response.raise_for_status()
        items_key = f"{type}s"; items = response.json().get(items_key, {}).get("items", [])
        return items[0] if items else None
    except (requests.exceptions.RequestException, IndexError) as e: logger.error(f"Error searching Spotify {type} for '{query}': {e}"); return None
    ...
    if retry_state.outcome.failed:
        ...
        if isinstance(exc, requests.exceptions.HTTPError):
            if exc.response is not None and exc.response.status_code in [400, 404]:
                args_info = retry_state.args[1:] if len(retry_state.args) > 1 else 'N/A'
                logger.warning(f"Spotify recommendations: Not retrying for HTTP {exc.response.status_code} (seeds: {args_info})")
                return False
    return True

@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10), retry=Sould_retry_spotify_recs)
...
async def generate_chat_response(user_id: int, message: str) -> str:
    ...
    if context_data.get("spotify", {}).get("recently_played"):
        artists = list(set(item["track"]["artists"][0]["name"] for item in context_data["spotify"]["recently_played"][:3] if item.get("track") and item["track"].get("artists")))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    spotify_linked = await ensure_spotify_token_valid(user_id)
...
async def download_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        caption = f"🎵 {dl_info['title']}" + (f"\n🎤 Artist: {dl_info['artist']}" if dl_info.get("artist") != "Unknown_Artist" else "") + (f"\n⏱️ {int(divmod(dl_info.get('duration',0),60)[0])}:{int(divmod(dl_info.get('duration',0),60)[1]):02d}" if dl_info.get("duration") else "")
...
async def link_spotify(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not all([SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_REDIRECT_URI]) or SPOTIFY_REDIRECT_URI == "https://your-callback-url.com": # Stricter check
        await update.message.reply_text("Spotify linking not properly configured by admin. Please ensure SPOTIFY_REDIRECT_URI is set correctly."); return ConversationHandler.END
...
async def send_search_results_keyboard(reply_target, query: str, results: List[Dict]) -> None:
    ...
    for i, res in enumerate(results[:5]):
        ...
        dur_val = res.get('duration', 0) # Ensure duration is treated as number
        dur_str = f" [{int(dur_val//60)}:{int(dur_val%60):02d}]" if dur_val else ""
...
async def recommend_music(update: Update, context: ContextTypes.DEFAULT_TYPE, from_mood_setter: bool = False) -> Optional[int]:
    ...
    if not spotify_linked_and_valid:
        await msg_obj.reply_text(
            ...
            quote=False # Don't quote the previous message if it was a callback edit
        )
    ...
    try:
        ...
        if spotify_linked_and_valid:
            logger.info(f"Updating Spotify data for user {user_id} for recommendation.")
            async def fetch_spotify_data_concurrently():
                tasks = [
                    get_user_spotify_data(user_id, "player/recently-played"),
                    get_user_spotify_data(user_id, "top/tracks?time_range=short_term")
                ]
                results = await asyncio.gather(*tasks, return_exceptions=True)
                if not isinstance(results[0], Exception) and results[0] is not None: user_contexts[user_id]["spotify"]["recently_played"] = results[0]
                if not isinstance(results[1], Exception) and results[1] is not None: user_contexts[user_id]["spotify"]["top_tracks"] = results[1]
            await fetch_spotify_data_concurrently()
        ...
        cur_artists_names = ai_analysis.get("artists", [])
        ...
        if not cur_mood and not from_mood_setter:
            ...
            return await set_mood(update, context)
        ...
        if spotify_linked_and_valid:
            ...
            if len(s_tracks) < 5 and spotify_user_data.get("top_tracks"): s_tracks.extend([t["id"] for t in spotify_user_data["top_tracks"][:max(0, 5-len(s_tracks))] if t.get("id")]) # Allow more track seeds
            
            if cur_artists_names and (len(s_tracks) + len(s_artists_ids)) < 5 :
                sp_token_for_search = await get_spotify_token()
                if sp_token_for_search:
                    for name in cur_artists_names[:max(0, 5 - len(s_tracks) - len(s_artists_ids))]:
        ...
        if spotify_linked_and_valid and (s_tracks or s_artists_ids or s_genres_for_api):
            ...
            sp_token_for_recs = await get_spotify_token()
...
async def enhanced_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
    ...
    if data.startswith("mood_"):
        ...
        except (TimedOut, NetworkError) as te: logger.error(f"Net/Timeout sending {vid_id}: {te}", exc_info=True); err="❌ Failed send (network)."; await (query.edit_message_text(err) if query.message else context.bot.send_message(chat_id=user_id, text=err))
...
async def enhanced_handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # ... (main try block logic as before) ...
        ...
        if len(text.split()) > 1 or detected_song_regex == "AI_ANALYSIS_NEEDED":
            ...
            txt_lower_for_lyrics = text.lower(); actual_song_q_lyrics = None 
            for kw in lyrics_kws: 
                if kw in txt_lower_for_lyrics:
                    parts = txt_lower_for_lyrics.split(kw,1)
            ...
            elif not ai_intent["is_lyrics_request"]: 
        ...
        txt_l_mood_detect = text.lower() 
        if "i'm feeling" in txt_l_mood_detect or "i feel" in txt_l_mood_detect:
            try: 
                split_phrase = "i'm feeling" if "i'm feeling" in txt_l_mood_detect else "i feel"
                mood_tok = txt_l_mood_detect.split(split_phrase,1)[1].strip().split()[0].rstrip('.,?!')
                if mood_tok in ["happy","sad","energetic","relaxed","focused","nostalgic","anxious","stressed","calm","excited"]: 
                    user_contexts[user_id]["mood"]=mood_tok
                    logger.info(f"Mood '{mood_tok}' from text for {user_id}")
            except IndexError:
                pass
        ...
        logger.warning(f"Send/edit failed for {user_id}: {re_err}")
        # No further reply here as the send itself failed
    except Exception as e: 
        logger.error(f"Error in enhanced_handle_message for {user_id}, text \"{text[:50]}...\": {e}", exc_info=True)
        try:
            await reply_with_retry(update.message, "Pardon, hit a snag. Try /help?") # This is the try that was misplaced
        except Exception: # If even sending error message fails
            logger.error(f"Failed to send error fallback message to user {user_id}")
    finally:
        if not typing_task.done(): 
            try:
                typing_task.cancel()
                await typing_task # Await cancellation
            except asyncio.CancelledError:
                logger.debug("Typing task successfully cancelled in finally block.")
            except Exception as e_cancel:
                logger.error(f"Error while cancelling typing_task: {e_cancel}")
...
def main() -> None:
    ...
    # Crucial check for SPOTIFY_REDIRECT_URI
    redirect_uri_env = os.getenv("SPOTIFY_REDIRECT_URI")
    if not redirect_uri_env or redirect_uri_env == "https://your-callback-url.com": 
        logger.critical("FATAL: SPOTIFY_REDIRECT_URI is not set or is the default placeholder. Spotify OAuth /link_spotify will NOT work. Please set it in your .env file and Spotify Developer Dashboard.")
        sys.exit(1)
