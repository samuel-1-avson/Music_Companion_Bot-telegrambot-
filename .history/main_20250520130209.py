...
def download_youtube_audio(url: str) -> Dict[str, Any]:
    ...
    try:
        ydl_opts = {
            'format': 'bestaudio[ext=m4a]/bestaudio[abr<=128]/bestaudio',  # Prefer lower bitrate
...
async def enhanced_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Union[int, None]:
    ...
    if data.startswith("mood_"):
        ...
        try:
            ...
            # Retry send_audio with increased timeout
            @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
            async def send_audio_with_retry():
                async with context.bot.request as request:
                    # Set a longer timeout for this request
                    request._client.timeout = 60  # 60 seconds
