...
# In main.py, add imports for redis
import redis
import threading
...
# Conversation states (ensure this matches your original code)
MOOD, PREFERENCE, ACTION, SPOTIFY_CODE = range(4)

# Update link_spotify function
async def link_spotify(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Initiate Spotify OAuth flow and poll for the authorization code."""
    if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET or not SPOTIFY_REDIRECT_URI:
        await update.message.reply_text("Sorry, Spotify linking is not available.")
    ...
    await update.message.reply_text(
        "Click the button below to link your Spotify account. "
        "Once authorized, I'll automatically link your account!",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
    
    # Start polling for the code
    context.job_queue.run_once(check_spotify_code, 5, data={"user_id": user_id}, name=f"spotify_{user_id}")
    return SPOTIFY_CODE

# Add check_spotify_code function
async def check_spotify_code(context: ContextTypes.DEFAULT_TYPE) -> None:
    job = context.job
    user_id = job.data["user_id"]
    code = redis_client.get(f"spotify_code:{user_id}")
    ...
    if code:
        ...
        if token_data and token_data.get("access_token"):
            ...
            await context.bot.send_message(
                chat_id=user_id,
                text="❌ Failed to link Spotify account. Try /link_spotify again."
            )
    else:
        # Retry polling if code not found yet, up to 60 seconds
        if job.schedule_removal_time is None or (datetime.now(pytz.UTC).timestamp() - job.created_at) < 60:
            context.job_queue.run_once(check_spotify_code, 5, data={"user_id": user_id}, name=f"spotify_{user_id}")
        else:
            await context.bot.send_message(
                chat_id=user_id,
                text="❌ Spotify linking timed out. Try /link_spotify again."
            )
async def check_spotify_code(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Check Redis for the Spotify authorization code."""
    job = context.job
    user_id = job.data["user_id"]
    code = redis_client.get(f"spotify_code:{user_id}")
    ...
    if code:
        ...
        # Retry polling for up to 60 seconds
        current_time = datetime.now(pytz.UTC).timestamp()
        job_time = job.schedule_removal_time if job.schedule_removal_time else current_time
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
    if context.args and context.args[0].startswith("spotify_code_"):
        code = context.args[0].split("spotify_code_")[1]
        context.args = [code]
        await spotify_code_command(update, context)
    else:
        await update.message.reply_text(
            f"Hi {update.effective_user.first_name}! 👋 I'm MelodyMind..."
        )
...
# Update the main function to remove any Flask-related code
def main() -> None:
    ...
    # Add handlers (keep your existing handlers)
