...
def get_user_spotify_data(user_id: int, endpoint: str) -> Optional[List[Dict]]:
    ...
    params = {"limit": 10}
     ...
     try:
        ...
        #     response.raise_for_status()
        response = requests.get(url, headers=headers, params=params) # If still sync
        response.raise_for_status()
        return response.json().get("items", [])
    except requests.exceptions.RequestException as e: # or httpx.RequestError / httpx.HTTPStatusError
        logger.error(f"Error fetching Spotify user data ({endpoint}): {e}")
        return [] # CHANGED: Return empty list for consistency if items are expected
...
async def recommend_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    client_credentials_token = await get_spotify_token()
    seed_track_ids = []
    user_s_data = user_contexts.get(user_id, {}).get("spotify", {})
    user_access_token = user_s_data.get("access_token")
    user_refresh_token_exists = bool(user_s_data.get("refresh_token"))
    if user_access_token and user_s_data.get("expires_at") and datetime.now(pytz.UTC).timestamp() > user_s_data["expires_at"]:
            if user_refresh_token_exists:
                logger.info(f"Spotify token expired for user {user_id}, attempting refresh.")
                # Assuming refresh_spotify_token is also made async:
                user_access_token = await refresh_spotify_token(user_id)
            else:
                user_access_token = None # Token expired, no way to refresh
    ...
    try:
        ...
        elif user_contexts.get(user_id, {}).get("spotify", {}).get("playlists") and user_access_token: # Ensure user_access_token
            playlist = user_contexts[user_id]["spotify"]["playlists"][0]
            playlist_id = playlist.get("id")
            if playlist_id:
                url = f"https://api.spotify.com/v1/playlists/{playlist_id}/tracks"
                # headers = {"Authorization": f"Bearer {token}"} # OLD, used client_credentials_token
                headers = {"Authorization": f"Bearer {user_access_token}"} # NEW: Use user's token
                # If using httpx for spotify calls:
                try:
                    async with httpx.AsyncClient() as http_client:
                        response = await http_client.get(url, headers=headers, params={"limit": 2})
                        response.raise_for_status()
                        if response.status_code == 200:
                            tracks_data = response.json().get("items", [])
                            seed_track_ids = [item["track"]["id"] for item in tracks_data if item.get("track", {}).get("id")]
                except httpx.HTTPStatusError as http_error:
                    logger.warning(f"Failed to fetch playlist tracks for {playlist_id} for user {user_id}: {http_error}")
                except httpx.RequestError as req_error:
                    logger.error(f"Request error fetching playlist tracks for user {user_id}: {req_error}")

        # Use client_credentials_token for general recommendations if seed_track_ids are found, or for search
            if client_credentials_token and seed_track_ids:
                recommendations = await get_spotify_recommendations(client_credentials_token, seed_track_ids[:2]) # assuming async 



        if client_credentials_token and seed_track_ids:
            recommendations = await get_spotify_recommendations(client_credentials_token, seed_track_ids[:2])
...
async def generate_chat_response(user_id: int, message: str) -> str:
    ...
    try:
        ...
        return reply
    except Exception as e: # Consider more specific OpenAI exceptions
        logger.error(f"Error generating chat response: {e}", exc_info=True) # Add exc_info for full traceback
...
async def download_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        async with download_lock: # Lock acquired again for removal
            if user_id in active_downloads:
                active_downloads.remove(user_id)
...
async def link_spotify(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    auth_url = (
        ...
        # "&scope=user-read-recently-played%20user-top-read"  # OLD
        "&scope=user-read-recently-played%20user-top-read%20playlist-read-private" # NEW: added playlist-read-private
        f"&state={user_id}" # It's good to have state, ensure you validate it on callback if possible
    )
    keyboard = [
