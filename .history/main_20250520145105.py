...
async def recommend_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if user_contexts.get(user_id, {}).get("spotify", {}).get("recently_played"):
            ...
            seed_track_ids = [track["track"]["id"] for track in tracks[:2] if track.get("威力

System: **Error**: The message you've provided exceeds the maximum length for a single message (65535 characters). Please reduce the length and try again.
...
async def smart_recommend_music(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Update Spotify data if available
        if user_contexts.get(user_id, {}).get("spotify"):
            recently_played = get_user_spotify_data(user_id, "player/recently-played")
            if recently_played:
                user_contexts[user_id]["spotify"]["recently_played"] = recently_played
                logger.debug(f"Updated recently played for user {user_id}: {len(recently_played)} tracks")
            top_tracks = get_user_spotify_data(user_id, "top/tracks")
            if top_tracks:
                user_contexts[user_id]["spotify"]["top_tracks"] = top_tracks
                logger.debug(f"Updated top tracks for user {user_id}: {len(top_tracks)} tracks")
        ...
        genres = analysis.get("genres", [])
        artists = analysis.get("artists", [])
        logger.debug(f"Conversation analysis for user {user_id}: mood={mood}, genres={genres}, artists={artists}")
        ...
        logger.debug(f"Seed track IDs: {seed_track_ids}")
        ...
        if token and seed_track_ids:
            ...
            if valid_seed_track_ids:
                logger.info(f"Attempting Spotify recommendations with seed tracks: {valid_seed_track_ids}")
                recommendations = get_spotify_recommendations(token, valid_seed_track_ids[:2])
                if recommendations:
                    logger.info(f"Received {len(recommendations)} Spotify recommendations")
                ...
                logger.warning("No valid Spotify seed tracks found")

        # Fallback to YouTube search
        results = search_youtube(search_query, max_results=5)
        logger.debug(f"YouTube search results: {results}")
        if results:
            response = f"🎵 <b>Recommended music for you:</b>\n\n"
            keyboard = []
            valid_results = []
            ...
            logger.warning(f"No YouTube results for query: {search_query}")
            await status_msg.delete()
            await provide_generic_recommendations(update, mood if mood else "happy")

    except Exception as e:
        logger.error(f"Error in smart_recommend_music: {e}", exc_info=True)
