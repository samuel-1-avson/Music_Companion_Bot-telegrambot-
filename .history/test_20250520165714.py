...
def search_spotify_track_sync(token: str, query: str, type: str = "track") -> Optional[Dict]: # Added type parameter
    ...
    try:
        ...
        response.raise_for_status()
        # The structure of the response depends on the search type
        items_key = f"{type}s" # e.g., "tracks", "artists"
...
def get_spotify_recommendations_sync(token: str, seed_tracks: Optional[List[str]] = None, seed_artists: Optional[List[str]] = None, seed_genres: Optional[List[str]] = None, limit: int = 5) -> List[Dict]:
    ...
    if not seed_tracks and not seed_artists and not seed_genres:
        logger.warning("No seeds (tracks, artists, or genres) provided for Spotify recommendations.")
        return []
    ...
    # Prioritize tracks, then artists, then genres for filling up to 5 seeds
    if seed_tracks:
        tracks_to_seed = seed_tracks[:max(0, 5 - total_seeds)]
        if tracks_to_seed: params["seed_tracks"] = ",".join(tracks_to_seed); total_seeds += len(tracks_to_seed)
    
    if seed_artists and total_seeds < 5:
        artists_to_seed = seed_artists[:max(0, 5 - total_seeds)]
        if artists_to_seed: params["seed_artists"] = ",".join(artists_to_seed); total_seeds += len(artists_to_seed)

    if seed_genres and total_seeds < 5:
        genres_to_seed = seed_genres[:max(0, 5 - total_seeds)]
        if genres_to_seed: params["seed_genres"] = ",".join(genres_to_seed); total_seeds += len(genres_to_seed)
            
    if total_seeds == 0:
        logger.warning("No valid seeds could be prepared for Spotify recommendations.")
        return []
    ...
    try:
        ...
        # Logged by retry condition, but can add more detail if it escapes tenacity
        # If this is reached, it means tenacity gave up or it's an error not handled by Sould_retry_spotify_recs
        logger.error(f"Final HTTPError for Spotify recommendations (params: {params}): {http_error.response.status_code if http_error.response else 'N/A'} - {http_error.response.text if http_error.response else 'No response text'}")
        return [] # Return empty if tenacity failed
    except requests.exceptions.RequestException as req_error:
        logger.error(f"Final RequestException for Spotify recommendations (params: {params}): {req_error}")
        return []
...
def get_user_spotify_token_sync(code: str) -> Optional[Dict]:
    if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET or not SPOTIFY_REDIRECT_URI:
        ...
        return None
    url = "https://accounts.spotify.com/api/token"
    headers = {"Authorization": f"Basic {base64.b64encode(f'{SPOTIFY_CLIENT_ID}:{SPOTIFY_CLIENT_SECRET}'.encode()).decode()}", "Content-Type": "application/x-www-form-urlencoded"}
...
def refresh_spotify_token_sync(user_id: int) -> Optional[str]:
    ...
    refresh_token = context_data.get("spotify", {}).get("refresh_token")
...
def get_user_spotify_data_sync(user_id: int, endpoint: str) -> Optional[List[Dict]]:
    context_data = user_contexts.get(user_id, {})
    spotify_data = context_data.get("spotify", {})
    access_token, expires_at = spotify_data.get("access_token"), spotify_data.get("expires_at")
...
def get_user_spotify_playlists_sync(user_id: int) -> Optional[List[Dict]]:
    # This is very similar to get_user_spotify_data_sync, could be merged if endpoint is the only diff
    context_data = user_contexts.get(user_id, {})
    spotify_data = context_data.get("spotify", {})
    access_token, expires_at = spotify_data.get("access_token"), spotify_data.get("expires_at")
    if not access_token or (expires_at and datetime.now(pytz.UTC).timestamp() > expires_at):
...
async def analyze_conversation(user_id: int) -> Dict:
    ...
    try:
        ...
        in_artists = [str(a) for a in result.get("artists", []) if isinstance(a, str)][:3]
...
def detect_music_in_message(text: str) -> Optional[str]: # Simplified; AI handles complex cases
    text_lower = text.lower()
    patterns = [r'(?:play|download|get|song)\s+(.+?)(?:\s+by\s+(.+))?$',] # Simpler pattern
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f"Hi {user.first_name}! 👋 I'm MelodyMind. I can download music, find lyrics, recommend tunes, and chat! Try /help for commands.")
...
async def recommend_music(update: Update, context: ContextTypes.DEFAULT_TYPE, from_mood_setter: bool = False) -> Optional[int]:
    ...
    try:
        ...
        if user_contexts[user_id].get("spotify", {}).get("access_token"):
            ...
            async def fetch_spotify_data(): # Helper to run concurrently if needed
                ...
                tt = await get_user_spotify_data(user_id, "top/tracks?time_range=short_term") # More current
                if tt is not None: user_contexts[user_id]["spotify"]["top_tracks"] = tt
                # Playlists not directly used for seeds here, but good for AI context
        ...
        if not cur_mood and not from_mood_setter:
            await status_msg.delete(); await msg_obj.reply_text("Need your mood for recs. Let's set it!"); return await set_mood(update, context)

        s_tracks, s_artists_ids, s_genres = [], [], cur_genres # Use AI genres for Spotify genre seeds
        ...
        # Attempt to get artist IDs for AI-inferred artist names
        if cur_artists_names and len(s_tracks) < 2 : # Only if we need more seeds
            client_token = await get_spotify_token()
            if client_token:
                for name in cur_artists_names[:2]: # Max 2 artist seeds from names
                    artist_data = await search_spotify_track(client_token, f"artist:{name}", type="artist")
                    if artist_data and artist_data.get("id"): s_artists_ids.append(artist_data["id"])
                    if len(s_artists_ids) >= 2 : break
        
        if s_tracks or s_artists_ids or s_genres:
            logger.info(f"Spotify recs for {user_id} with seeds: tracks={s_tracks}, artists={s_artists_ids}, genres={s_genres}")
            sp_token = await get_spotify_token()
            if sp_token:
                sp_recs = await get_spotify_recommendations(sp_token, seed_tracks=s_tracks, seed_artists=s_artists_ids, seed_genres=s_genres, limit=5)
                if sp_recs:
        ...
        logger.info(f"Falling back to YouTube search for {user_id} recommendation.")
        yt_query_parts = []
        if cur_mood: yt_query_parts.append(cur_mood)
        if cur_genres: yt_query_parts.append(cur_genres[0])
...
async def enhanced_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
    ...
    if data.startswith("mood_"):
        ...
        except (TimedOut, NetworkError) as te: logger.error(f"Net/Timeout sending {vid_id}: {te}", exc_info=True); err="❌ Failed send (network)."; try: await query.edit_message_text(err)
        except Exception as e: logger.error(f"Error in button dl {vid_id}: {e}", exc_info=True); err=f"❌ Error: {str(e)[:100]}"; try: await query.edit_message_text(err)
        finally:
            if user_id in active_downloads: active_downloads.remove(user_id)
...
async def enhanced_handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if detected_song_regex and detected_song_regex != "AI_ANALYSIS_NEEDED":
            await typing_task; status_msg = await reply_with_retry(update.message, f"🔍 You want '{detected_song_regex}'. Searching YouTube...")
            results = await search_youtube(detected_song_regex,max_results=3); await status_msg.delete()
            if results: await send_search_results_keyboard(update.message,detected_song_regex,results)
            else: await reply_with_retry(update.message, f"Sorry, couldn't find '{detected_song_regex}' on YouTube."); return
        
        ai_intent = {"is_music_request": False, "song_query": None, "is_lyrics_request": False}
        if len(text.split()) > 1 or detected_song_regex == "AI_ANALYSIS_NEEDED": # Min 2 words for AI, or if regex flagged
            lyrics_kws = ["lyrics for", "lyrics to", "words to", "words for", "lyrics of", "lyrics"]
            txt_lower = text.lower(); actual_song_q_lyrics = None
            for kw in lyrics_kws:
                if kw in txt_lower: actual_song_q_lyrics = txt_lower.split(kw,1)[-1].strip("?.! "); break
            if actual_song_q_lyrics: ai_intent["is_lyrics_request"], ai_intent["song_query"] = True, actual_song_q_lyrics
            elif not ai_intent["is_lyrics_request"]: # Only if not lyrics
                music_req_ai = await is_music_request(text)
                if music_req_ai["is_music_request"] and music_req_ai["song_query"]: ai_intent["is_music_request"], ai_intent["song_query"] = True, music_req_ai["song_query"]
        
        await typing_task # ensure awaited/cancelled
        if ai_intent["is_music_request"] and ai_intent["song_query"]:
            status_msg_ai = await reply_with_retry(update.message, f"🔍 Searching YouTube for '{ai_intent['song_query']}'...")
            ...
            else: await reply_with_retry(update.message, f"Sorry, couldn't find '{ai_intent['song_query']}' on YouTube."); return
        if ai_intent["is_lyrics_request"] and ai_intent["song_query"]: context.args=[ai_intent["song_query"]]; await get_lyrics_command(update,context); return
...
async def cancel_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.callback_query: await update.callback_query.answer(); try: await update.callback_query.edit_message_text(msg)
