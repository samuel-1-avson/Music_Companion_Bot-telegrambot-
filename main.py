...
async def send_audio_file(update: Update, file_path: str, title: str, artist: str):
...
async def handle_download_error(update: Update, error: Exception):
    """Handle different download error scenarios."""
    error_msg = str(error).lower()
    
    if "private" in error_msg:
        await update.message.reply_text("🔒 This video is private.")
    elif "copyright" in error_msg:
        await update.message.reply_text("⚠️ Copyright protected content.")
    elif "unavailable" in error_msg:
        await update.message.reply_text("❌ Video unavailable.")
    else:
        await update.message.reply_text(f"❌ Error: {str(error)[:200]}")
